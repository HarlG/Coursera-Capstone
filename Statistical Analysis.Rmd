...
```{r}
mod1 = model.matrix( ~ pdata$Group)
fit1 = lmFit(filt_edata, mod1)
fit1 = eBayes(fit1)
tt = topTable(fit1, number = 21030, p.value = 1)
...
genes_logFC$diffexp[genes_logFC$logFC > 1.5 & genes_logFC$P.Value < 0.05] = "UP"
genes_logFC$diffexp[genes_logFC$logFC < -1.5 & genes_logFC$P.Value < 0.05] = "DOWN"
...
ggplot(data=genes_logFC, aes(x=logFC, y=-log10(P.Value), col=diffexp)) + 
    geom_point()
    theme_minimal()
```
